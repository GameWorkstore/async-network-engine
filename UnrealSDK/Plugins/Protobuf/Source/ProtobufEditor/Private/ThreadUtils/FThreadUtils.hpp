...
class FProtobufThread : public FRunnable
{
public:
	using FCallback = TFunction<void()>;
	explicit FProtobufThread(const TCHAR *InThreadName, const FCallback& InRunFunc)
...
private:
	FProtobufThread(const FProtobufThread&) = delete;
	FProtobufThread& operator=(const FProtobufThread&) = delete;
